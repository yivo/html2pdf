...
A Node.js microservice that converts HTML content to PDF using `wkhtmltopdf` under the hood.

**Note:** This service is designed to be run as a Docker container. We use the official `surnet/alpine-node-wkhtmltopdf` image as our base.

## Features

- REST API for HTML to PDF conversion
- Authentication via JWT (JSON Web Tokens)
- Support for extensive `wkhtmltopdf` options (margins, headers, footers, TOC, JS execution, etc.)
...
| --- | --- | --- | --- |
| `JWT_SECRET` | Secret key used to verify the JWT token for authentication. | - | **Yes** |
| `APP_PORT` | The port on which the server will listen. | `8080` | No |
| `WKHTMLTOPDF_TIMEOUT` | Timeout in milliseconds for the `wkhtmltopdf` process. | `1800000` (30 mins) | No |

## Authentication

The API requires authentication using a JSON Web Token (JWT). The token must be signed using the `HS256` algorithm with the secret specified in the `JWT_SECRET` environment variable.

Send the token in the `Authorization` header of your requests:

```
Authorization: Bearer <your_jwt_token>
```

If the token is missing, invalid, revoked, or expired, the service will return a `401 Unauthorized` status.
...
Converts the provided HTML content and options into a PDF.

#### Request Body (JSON)

The request body must be a JSON object containing the HTML to convert and any desired `wkhtmltopdf` options. The maximum request body size is limited to `32mb`.

**Important:** For `body_html`, `header_html`, and `footer_html`, you must provide a full HTML document including the `<!DOCTYPE html>`, `<html>`, `<head>`, and `<body>` tags.

**Required Parameter:**

- `body_html` (String): The main HTML content to be converted to PDF. (Must be a full HTML document).

**Optional Parameters (wkhtmltopdf options):**

The service supports various `wkhtmltopdf` options. Options are passed as JSON keys formatted with underscores instead of hyphens (e.g., `--page-size` becomes `page_size`).

**1. Boolean Options (set to `true` to enable):**
e.g., `"collate": true`, `"grayscale": true`, `"disable_javascript": true`
*Supported:* `collate`, `no_collate`, `grayscale`, `lowquality`, `no_pdf_compression`, `read_args_from_stdin`, `use_xserver`, `dump_default_toc_xsl`, `outline`, `no_outline`, `background`, `no_background`, `custom_header_propagation`, `no_custom_header_propagation`, `debug_javascript`, `no_debug_javascript`, `default_header`, `disable_external_links`, `enable_external-links`, `disable_forms`, `enable_forms`, `images`, `no_images`, `disable_internal_links`, `enable_internal_links`, `disable_javascript`, `enable_javascript`, `keep_relative_links`, `disable_local_file_access`, `enable_local_file_access`, `exclude_from_outline`, `include_in_outline`, `disable_plugins`, `enable_plugins`, `print_media_type`, `no_print_media_type`, `proxy_hostname_lookup`, `resolve_relative_links`, `disable_smart_shrinking`, `enable_smart_shrinking`, `stop_slow_scripts`, `no_stop_slow_scripts`, `disable_toc_back_links`, `enable_toc_back_links`, `footer_line`, `no_footer_line`, `header_line`, `no_header_line`, `disable_dotted_lines`, `disable_toc_links`

**2. Value Options (String/Number):**
e.g., `"page_size": "A4"`, `"margin_top": "10mm"`, `"zoom": 1.5`
*Supported:* `copies`, `dpi`, `image_dpi`, `image_quality`, `margin_bottom`, `margin_left`, `margin_right`, `margin_top`, `orientation`, `page_height`, `page_size`, `page_width`, `title`, `outline_depth`, `bypass_proxy_for`, `encoding`, `javascript_delay`, `load_error_handling`, `load_media_error_handling`, `minimum_font_size`, `page_offset`, `password`, `proxy`, `run_script`, `ssl_key_password`, `username`, `viewport_size`, `window_status`, `zoom`, `footer_center`, `footer_font_name`, `footer_font_size`, `footer_left`, `footer_right`, `footer_spacing`, `header_center`, `header_font_name`, `header_font_size`, `header_left`, `header_right`, `header_spacing`, `toc_header_text`, `toc_level_indentation`, `toc_text_size_shrink`

**3. Name-Value Options (Array of Arrays `[[name, value]]`):**
e.g., `"cookie": [["session_id", "12345"]]`
*Supported:* `cookie`, `custom_header`, `post`, `replace`

**4. File Path Options (String containing file content):**
The service handles saving this content to temporary files automatically. (For HTML files, must be a full HTML document).
e.g., `"header_html": "<!DOCTYPE html><html><body>Header</body></html>"`
*Supported:* `header_html`, `footer_html`, `checkbox_checked_svg`, `checkbox_svg`, `radiobutton_checked_svg`, `radiobutton_svg`, `ssl_crt_path`, `user_style_sheet`, `xsl_style_sheet`, `ssl_key_path`, `cookie_jar`

**5. File Name + File Path Options (Array of Arrays `[[filename, filecontent]]`):**
*Supported:* `post_file`
...
curl -X POST http://localhost:8080/ \
  ...
  -d '{
    ...
    "margin_top": "20mm",
    "margin_bottom": "20mm",
    "header_html": "<!DOCTYPE html>\n<html>\n<head>\n</head>\n<body>\n<div style=\"text-align: center;\">My Custom Header</div>\n</body>\n</html>",
    "grayscale": true
  }' \
  --output result.pdf
```
...
- **Success (200 OK):** Returns the generated PDF file (`application/pdf`).
- **Unprocessable Entity (422):** Returned if `body_html` is missing or empty.
- **Unauthorized (401):** Returned if the JWT token is missing, invalid, revoked, or expired.
- **Internal Server Error (500):** Returned if an error occurs during PDF generation.

---

## wkhtmltopdf Help Output

Below is the full help output from the underlying `wkhtmltopdf` tool, which can be useful as a reference for the available options and their behavior:
